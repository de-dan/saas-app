...
const Profile = async () => {
  ...
  console.log('companins', companions)
  ...
  return <main className="min-lg:w-3/4">
    <section className='flex justify-between gap-4 max-sm:flex-col items-center'>
      <div className='flex gap-4 items-center'>
        ...
        <div className='flex flex-col gap-2'>
          <h1 className='font-bold text-2xl'>{[user.firstName, user.lastName].join(' ')}</h1>
    ...
    <Accordion type='multiple'>
      <AccordionItem value='recent'>
        ...
        <AccordionContent>
          <CompanionsList title='Recent Sessions' classNames='' companions={sessionHistory}/>
        </AccordionContent>
      </AccordionItem>
      <AccordionItem value='companions'>
        <AccordionTrigger className='text-2xl font-bold'>My Companions {`(${companions.length})`}</AccordionTrigger>
        <AccordionContent>
          <CompanionsList title='My Companions' classNames='' companions={companions}/>
        </AccordionContent>
      </AccordionItem>
    </Accordion>
  </main>;
